...
/**
     ...
     * Register Domain
     *
     * @param  \Alexasomba\\Paystack\Model\ApplePayParam $apple_pay_param apple_pay_param (optional)
     ...
     * Register Domain
     *
     * @param  \Alexasomba\\Paystack\Model\ApplePayParam $apple_pay_param (optional)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['applePayRegisterDomain'] to see the possible values for this operation
     *
     * @throws \Alexasomba\\Paystack\ApiException on non-2xx response
     * @throws \InvalidArgumentException
    ...
    public function applePayRegisterDomainWithHttpInfo($apple_pay_param = null, string $contentType = self::contentTypes['applePayRegisterDomain'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Alexasomba\\Paystack\Model\ApplePayCreateOkModel' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Alexasomba\\Paystack\Model\ApplePayCreateOkModel' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\Alexasomba\\Paystack\Model\ApplePayCreateOkModel', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 401:
