...
/**
     ...
     *
     * @param  string $cursor The cursor value of the next set of authorizations to fetch. You can get this from the meta object of the response (optional)
     * @param  string $status Filter by the authorization status (optional)
     * @param  int $per_page The number of authorizations to fetch per request (optional)
    ...
    public function directdebitTriggerActivationChargeWithHttpInfo($direct_debit_activation_charge_request, string $contentType = self::contentTypes['directdebitTriggerActivationCharge'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Alexasomba\\Paystack\Model\DirectDebitActivationChargeResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Alexasomba\\Paystack\Model\DirectDebitActivationChargeResponse' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\Alexasomba\\Paystack\Model\DirectDebitActivationChargeResponse', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 401:
